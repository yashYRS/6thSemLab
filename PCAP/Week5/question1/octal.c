...
int main(void) {
	...
	cl_mem res_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, LIST_SIZE*sizeof(int), NULL, &ret ) ;

	ret = clEnqueueWriteBuffer(cmd_q, a_mem_obj, CL_TRUE,0, LIST_SIZE*sizeof(int), A,0,NULL, NULL) ;
	ret = clEnqueueWriteBuffer(cmd_q, res_mem_obj, CL_TRUE, 0,LIST_SIZE*sizeof(int), Res,0,NULL, NULL) ;
	...
	ret = clEnqueueReadBuffer(cmd_q, res_mem_obj, CL_TRUE, 0, LIST_SIZE*sizeof(int), Res, 0, NULL, NULL) ;

	for (i = 0 ; i < LIST_SIZE ; i++)
		printf(" Octal for %d - %d  \n", A[i], Res[i]) ;
